...
For every pet_resampled.nii.gz:
  ...
  2. Normalize clipped SUV values to [0, 1].
  3. Save pet_norm.nii.gz in the same PET folder.
...
def preprocess_pet_nii(
    ...
    """Clip and normalize all resampled PET SUV NIfTI files under root_dir.
    ...
    for index, (study_dir, pet_path) in enumerate(pet_items, start=1):
        ...
        try:
            ...
            pet_norm = _clip_and_normalize(pet_suv, lower, upper).astype(out_dtype, copy=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Clip SUV PET NIfTI files and normalize them to [0, 1]."
    )
